...
public class configSpecificTimeAndAnytime extends browserSetup {
    public void negativeCase() throws InterruptedException {
        String orderTime = "asap";
        String orderMode = "Dine In";
        new orderMode(orderMode);
        String slot = new selector().handleSelector(orderMode, "Eleventh Location", orderTime, null);
        if(slot.equalsIgnoreCase("Store is Closed at the Moment! Skipping Cart Creation")){
            System.out.println("CASE: PASS: Store is not Available Outside Set Specific Time, Weekly Fixed");
        }
    }
    ...
    public void positiveCase() throws InterruptedException {
        ...
        String orderMode = "Dine In";
        new orderMode(orderMode);
        new selector().handleSelector(orderMode, "Twelfth Location", orderTime, null);
        new addMenuItem();
        new cartHeader();
        new guestCheckout();
