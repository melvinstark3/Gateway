...
public class pickupSameDayPreOrder extends browserSetup {
    public pickupSameDayPreOrder() throws InterruptedException {
        String orderTime = "later";
        String orderMode = "Dine In";
        new orderMode(orderMode);
        new selector().handleSelector(orderMode, "Fifth Location", orderTime, null);
        new addMenuItem();
        new cartHeader();
        new guestCheckout();
        ...
        System.out.println("CASE: Later Timing with Same Day Slot (Later Setting: Allow pre (advance) orders), ETA(in Minutes), Pick Up Mode");
