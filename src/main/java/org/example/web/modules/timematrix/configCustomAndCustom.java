...
public class configCustomAndCustom extends browserSetup {
    public void negativeCase() throws InterruptedException {
        String orderTime = "asap";
        String orderMode = "Dine In";
        new orderMode(orderMode);
        String slot = new selector().handleSelector(orderMode, "Twenty First Location", orderTime, null);
    ...
    public void checkAvailableDays(){
        String orderMode = "Dine In";
        new orderMode(orderMode);
        Set<String> enabledWeekDays = new selector().checkDates("Twenty Second Location");
        Set<String> requiredDays = Set.of("MONDAY", "TUESDAY", "WEDNESDAY");

        if (enabledWeekDays.containsAll(requiredDays)) {
            System.out.println("CASE: PASS: Store is Open as per Custom Days");
        } else {
            System.out.println("CASE: FAIL: Store is Open but not as per Custom Days");
        }
    }
    ...
    public void positiveCase() throws InterruptedException {
        ...
        String orderMode = "Dine In";
        new orderMode(orderMode);
        new selector().handleSelector(orderMode, "Twenty Second Location", orderTime, null);
        new addMenuItem();
        new cartHeader();
        new guestCheckout();
        ...
        System.out.println("Schedule Logic CONFIG:\nPlacing Order Between: Specific Time, Weekly Fixed");
        System.out.println("Allow pickup / delivery Between: Custom Time");
        try {
